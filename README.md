...
```
jd-relay/
├── src/forwarder/              # Python Forwarder (新)
│   ├── main.py                 #   入口
...
│   ├── crypto/                 #   加密模块 (与 C++ 互通)
│   ├── ws/                     #   WebSocket 服务端
│   ├── api/                    #   REST API (钉钉回调 / 管理面板)
│   └── services/               #   业务逻辑 (钉钉 SDK / RelayService)
├── agent/                      # C++ Agent
│   ├── crypto/                 #   加密模块 (OpenSSL)
│   ├── protocol/               #   握手协议
...
# 生成密钥
python -m forwarder.main --gen-keys

# 运行（开发模式）
python -m forwarder.main

# 运行测试
pytest tests/python/ -v   # 65/65 passed
```
...
# 运行测试
cd build_wsl && ctest --output-on-failure   # 43/43 passed

# 生成密钥
./build_wsl/bin/keygen /tmp/keys
